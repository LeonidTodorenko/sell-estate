...
const [uploading, setUploading] = useState(false);
  ...
  const initializeForm = useCallback(() => {
    ...
    try {
      const res = await api.get(`/properties/${existing.id}/media`);
    
      const normalized: PropertyMedia[] = (res.data ?? []).map((m: any) => ({
        id: m.id,
...
const deleteMedia = async (mediaId: string) => {
  try {
    await api.delete(`/properties/media/${mediaId}`);
    if (existing?.id) {
      const listRes = await api.get(`/properties/${existing.id}/media`);
    const normalized: PropertyMedia[] = (listRes.data ?? []).map((m: any) => ({
        id: m.id,
...
> 
  ...
  <View style={styles.mediaBlock}>
    <Text style={styles.mediaTitle}>Video files</Text>

    <BlueButton
      title={uploading ? 'Uploading...' : 'Upload image/video'}
      onPress={pickAndUploadMedia}
      disabled={uploading}
    />

    {media.length === 0 ? (
      <Text style={{ marginTop: 10, color: '#666' }}>No media uploaded yet.</Text>
    ) : (
            ...
            <TouchableOpacity
              onPress={() =>
                Alert.alert('Delete', 'Remove this media?', [
                  ...
                  { text: 'Delete', style: 'destructive', onPress: () => deleteMedia(m.id) },
                ])
              }
